...
// ── "Hear it" · real chord playback through the SoundFont MIDI player ──
// Same Magenta <midi-player> the exercise pages use. We compute the chord's
// sounding notes from the recommended voicing's frets + standard tuning, wrap
// them in a tiny in-memory MIDI file, and strum it on an acoustic guitar.
...
function voicingMidi(fingering: (number | "x")[]): number[] {
  const out: number[] = [];
...
function HearItButton({ fingering }: { fingering: (number | "x")[] }) {
  ...
  // Pull in the (heavy) SoundFont player lazily, only once a reveal is shown.
  useEffect(() => {
    let ok = true;
  ...
  return (
    <>
      ...
      <button className="btn small" style={{ marginTop: 10, borderRadius: 999 }} onClick={play}>
        🔊 Hear it
      </button>
    </>
  );
