...
- [~] **Deferred follow-ups:** **(done)** unattended Square OAuth token refresh — `processSquareTokenRefresh` (`lib/square-token-refresh.ts`, pure + injectable, unit-tested) runs from the Worker cron alongside lesson reminders, refreshing tokens within 7 days of expiry (idempotent; no-op unless a Square coach is in-window or creds unset). **(done)** catalog-plan reuse — the catalog upsert idempotency key is now deterministic on (cadence, price) within the coach's own Square account, so repeated checkouts reuse one plan instead of minting per-checkout (degrades gracefully if Square drops the idem record). **Still deferred:** Square refund UI (parity with the still-mockup-blocked Stripe refund UI).

---

## 6. Notifications (inbox + email)

- [x] Email: booking confirmation, booking cancellation, practice notes, password reset — sent via the **Cloudflare Email Sending binding** (`env.EMAIL`, `[[send_email]]` in `wrangler.toml`). Migrated off Resend (no API key); `email.service.ts` calls `env.EMAIL.send({ from, to, subject, html, text })`, deriving a plain-text part from the HTML for spam scoring. Binding is absent in Node dev/tests, so sends are skipped + logged there. **Domain `usesunbird.com` onboarded to Email Sending (2026-06-15) and live delivery verified** via a real test send from `noreply@usesunbird.com`.
...
- [x] Loading / error states across the app — reusable `ui/Spinner` (+`LoadingState`) and `ui/ErrorState` (inline/block + retry) wired into the silent-catch offenders: Home categories, Lessons, Coaches (was showing "no coaches" on error), PracticeCalendar, **MyInbox** (was misleading "Inbox is quiet" on error → now error+retry, desktop + mobile), and **TodayPage** (dashboard error+retry; `useMyStudentDetail` now exposes an additive `error` flag). Good pre-existing handling left as-is (LessonDetail, MyGoals, StudentSession, StepConfirm).
...
- [~] Production env: Cloudflare secrets for Stripe, Calls; ~~onboard the email domain~~ (**done** — `usesunbird.com` enabled for Email Sending, live send verified 2026-06-15); D1 prod database; domain + SSL
