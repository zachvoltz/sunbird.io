...
- [ ] Coach UI to create/edit `SubscriptionPlan` rows (N-per-month packages) — **planned near-term, mockup-gated.** Confirmed (2026-06-02) we will sell lesson packages soon, so this **coexists** with the shipped per-session model rather than replacing it. `SubscriptionPlan` + `Subscription` are the foundation (currently orphaned: no API/UI/webhook usage); `LessonType.pricePerSession` is reserved for the same work. Model reconciliation + build outline (plan CRUD → Stripe Price sync → student plan-picker → `lessonsUsedThisPeriod` credit tracking → reschedule/cancel semantics → webhook wiring) in `pricing-model-design-note.md`. **Decide coexistence first**: recommend a per-coach toggle (sells per-session *or* packages, not both at once).
